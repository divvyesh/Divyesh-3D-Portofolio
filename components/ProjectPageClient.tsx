...
function SectionLabel({ icon, label, color = 'var(--glass-low)' }: { icon: React.ReactNode; label: string; color?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '18px' }}>
      <span style={{ color, display: 'flex', alignItems: 'center', opacity: 0.85 }}>{icon}</span>
      <span style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.14em', color, fontWeight: 500 }}>
        {label}
      </span>
    </div>
  )
...
function SlideViewer({ pdfUrl }: { pdfUrl: string }) {
  ...
  const btnStyle: React.CSSProperties = {
    ...
    border: '1px solid rgba(255,255,255,0.14)', background: 'rgba(6,7,13,0.82)',
    color: 'var(--glass-mid)', fontSize: '22px', cursor: 'pointer',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    backdropFilter: 'blur(8px)', lineHeight: 1, paddingBottom: '2px',
  }
  return (
    <div>
...
const glass: React.CSSProperties = { background: 'var(--glass-bg)', backdropFilter: 'var(--glass-blur)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '24px', marginBottom: '14px' }
const redGlass: React.CSSProperties = { background: 'rgba(110,0,0,0.28)', backdropFilter: 'blur(22px) saturate(150%)', border: '1px solid rgba(220,38,38,0.26)', borderLeft: '3px solid rgba(220,38,38,0.62)', borderRadius: '14px', padding: '24px', marginBottom: '14px' }
const tealGlass: React.CSSProperties = { background: 'rgba(0,85,60,0.18)', backdropFilter: 'blur(22px) saturate(150%)', border: '1px solid rgba(52,211,153,0.13)', borderLeft: '3px solid rgba(52,211,153,0.48)', borderRadius: '14px', padding: '24px', marginBottom: '14px' }
const greenGlass: React.CSSProperties = { background: 'rgba(0,60,22,0.26)', backdropFilter: 'blur(22px) saturate(150%)', border: '1px solid rgba(0,200,80,0.2)', borderLeft: '3px solid rgba(0,200,80,0.52)', borderRadius: '14px', padding: '24px', marginBottom: '14px' }
const innerCard: React.CSSProperties = { padding: '14px 16px', borderRadius: '10px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }
...
export default function ProjectPageClient({ project, prev, next }: { project: Project; prev: Project | null; next: Project | null }) {
  const html = v25Html[project.slug]
  ...
  return (
    <PageShell project={project} prev={prev} next={next}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {project.problemLines.map((line, i) => (
                <div key={i} style={{ display: 'flex', gap: '12px' }}>
                  <span style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: '10px', color: 'rgba(220,75,75,0.5)', flexShrink: 0, marginTop: '3px', minWidth: '20px' }}>{String(i + 1).padStart(2, '0')}</span>
                  <p style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '14px', lineHeight: 1.65, color: 'rgba(240,241,245,0.78)', margin: 0 }}><BoldText text={line} /></p>
                </div>
              ))}
            </div>
          ) : (
            <p style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '14px', lineHeight: 1.65, color: 'rgba(240,241,245,0.78)', margin: 0 }}><BoldText text={project.problem} /></p>
          )}
        </div>
      )}
      ...
      <div style={tealGlass}>
        ...
        <p style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '14px', lineHeight: 1.7, color: 'var(--glass-mid)', margin: 0 }}><BoldText text={project.decisionChanged} /></p>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '10px' }}>
            {project.analyses.map((a, i) => (
              <div key={i} style={{ ...innerCard, display: 'flex', gap: '12px' }}>
                ...
                <div>
                  <div style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7c5cff', marginBottom: '4px' }}>{a.title}</div>
                  <div style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '13px', lineHeight: 1.5, color: 'var(--glass-mid)' }}><BoldText text={a.finding} /></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={glass}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {[{ sub: 'Data Sources', text: project.data }, { sub: 'Methods Applied', text: project.method }, { sub: 'What I Did', text: project.whatIDid }, { sub: 'Result', text: project.result }].map(({ sub, text }) => (
            <div key={sub}>
              <div style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--glass-low)', marginBottom: '7px' }}>{sub}</div>
              <p style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '14px', lineHeight: 1.65, color: 'var(--glass-mid)', margin: 0 }}><BoldText text={text} /></p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={glass}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {project.stack.map(s => (
            <span key={s} style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: '11px', padding: '5px 12px', borderRadius: '9999px', background: 'rgba(124,92,255,0.1)', border: '1px solid rgba(124,92,255,0.22)', color: '#9b7cff' }}>{s}</span>
          ))}
        </div>
      </div>
      ...
      {(project.deckUrl || project.githubUrl) && (
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '14px' }}>
          {project.deckUrl && <a href={project.deckUrl} target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '14px', padding: '8px 16px', borderRadius: '8px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: 'var(--glass-mid)', textDecoration: 'none' }}>View deck &#8599;</a>}
          {project.githubUrl && <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '14px', padding: '8px 16px', borderRadius: '8px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: 'var(--glass-mid)', textDecoration: 'none' }}>GitHub &#8599;</a>}
        </div>
      )}
